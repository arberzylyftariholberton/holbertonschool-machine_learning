...
including forward propagation, cost calculation, and evaluation of predictions.
"""
import numpy as np
...
class NeuralNetwork:
    ...
    def evaluate(self, X, Y):
        """
        Evaluates the predictions of the neural network for given input data.
        ...
        Returns:
            tuple: (prediction, cost_value)
                - prediction (numpy.ndarray): Predicted labels for each example (0 or 1),
                                              shape (1, m). Labels are 1 if the output
                                              neuron’s activated output >= 0.5, else 0.
                - cost_value (float): Logistic regression cost for the predictions.
        """
