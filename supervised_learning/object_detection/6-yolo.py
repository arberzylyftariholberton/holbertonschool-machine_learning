...
Task 6: Show Boxes - annotates an image with bounding boxes, class names and
scores, displays it interactively, and optionally saves the result.
"""
import os
import cv2
import numpy as np
...
class Yolo:
    ...
    def preprocess_images(self, images):
        """
        Resizes images to the model's input dimensions
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for i, box in enumerate(boxes):
            ...
            # Draw bounding box with a blue line of thickness 2
            cv2.rectangle(annotated, (x1, y1), (x2, y2),
                          (255, 0, 0), 2)

            # Build the label string: class name and rounded score
            label = "{} {:.2f}".format(class_name, score)

            # Place label text 5 pixels above the top-left corner of the box
        ...
        # Display the window; title is the bare filename
        cv2.imshow(file_name, annotated)
        key = cv2.waitKey(0)

        if key == ord('s'):
            # Create the detections directory if it does not exist
