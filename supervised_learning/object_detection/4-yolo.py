...
Task 4: Load Images - loads all images from a folder into memory using OpenCV.
"""
import os
import cv2
import numpy as np
...
class Yolo:
    ...
    def load_images(folder_path):
        ...
        # Iterate over every entry in the given folder
        for filename in os.listdir(folder_path):
            full_path = os.path.join(folder_path, filename)
            img = cv2.imread(full_path)

            # Skip entries that OpenCV cannot decode as an image
            if img is not None:
                images.append(img)
